...
/** Chronological assessment list with per-item deletion and baseline-aware confirmation. */
export function AssessmentTimeline({ assessments, onRemove }: AssessmentTimelineProps) {
  ...
  const confirmingIndex = assessments.findIndex((a) => a.id === confirmingId);
  const isDeletingBaseline = confirmingIndex === 0;
        ...
        <div className="max-h-100 overflow-y-auto">
          {assessments.length === 0 ? (
            ...
            assessments.map((assessment, idx) => (
              <div
                key={assessment.id}
                className={`flex items-start justify-between p-3 border-b transition-colors ${
